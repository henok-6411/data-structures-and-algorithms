...
const splitFoods = (recipe) => {
  ...
  // Solution code here...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    let foodIndex = recipe.ingredients[i].split(' ');
    let foodSlice = foodIndex.slice(2);
    result.push(foodSlice.join(' '));
  }
...
const stepActions = (recipe) => {
  ...
  for (let i = 0; i < recipe.steps.length; i++) {
    let stepIndex = recipe.steps[i].split(' ');
    let spliceIndex = stepIndex.splice(0, 1);
    result.push(spliceIndex.join(' '));
...
const totalSumCSV = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    let strIndex = str[i].split(' ');
    let strNumber = strIndex.parseInt(str, 10);
    strNumber++;
